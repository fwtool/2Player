...
{
	ifstream fs;
    fs.open(util::WStrToStr(strFile), ios::binary);
	if (!fs || !fs.is_open())
	{
		return false;
	}
...
bool fsutil_win::copyFile(const string& strSrcFile, const string& strSnkFile, bool bSyncModifyTime)
{
	ifstream srcStream;
	try
	{
	...
	{
		struct _stat fileStat;
        if (0 == _stat(strSrcFile.c_str(), &fileStat))
		{
			struct _utimbuf timbuf { fileStat.st_atime, fileStat.st_mtime };
            (void)_utime(strSnkFile.c_str(), &timbuf);
		}
	}
